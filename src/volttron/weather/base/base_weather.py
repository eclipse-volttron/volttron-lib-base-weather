...
from volttron.utils import (fix_sqlite3_datetime,get_aware_utc_now, format_timestamp, jsonapi,
                            process_timestamp, parse_timestamp_string)
...
APIService = TypedDict('APIService', {'type': str, 'update_interval': timedelta | None, 'description': str})
...
class BaseWeatherAgent(Agent):
    ...
    def __init__(self,
        ...
        try:
            ...
            self._api_services: dict[str, APIService] = {
            ...
            self.vip.health.set_status(STATUS_BAD, f"Failed to load weather agent settings. Error: {e}")

    # Configuration methods

    def get_api_calls_interval(self) -> timedelta | None:
        """
        By default, sets the api call parameters for cache to None.
        Should be overridden by concrete agents that utilize an api call tracking feature.
        :return: Datetime timedelta representing the period for which api calls are measured,
                 Number of api calls allotted for the given period
        """
        return None

    def register_service(self, service_function_name, interval, service_type, description=None):
        """Called in a weather agent's __init__ function to add api service to the api services dictionary.
        ...
        :param service_type: str  Determines the structure of the cached data: "history", "current", or "forecast".
        ...
        if description and not isinstance(description, str):
            raise ValueError("description is expected as a string describing the service function's usage.")
    ...
    def remove_service(self, service_function_name):
        ...
        if service_function_name in self._api_services:
            ...
            raise ValueError(f"service {service_function_name} does not exist")
    ...
    def validate_location_dict(self, service_name, location):
        ...
        if not isinstance(location, dict):
            record_dict = {
                "location": location,
                WEATHER_ERROR: "Invalid location format. Location should be specified as a dictionary"
            }
    ...
    def get_update_interval(self, service_name):
        ...
        :param service_name: name of service for which to retrieve the update interval
    ...
    def get_api_description(self, service_name):
        ...
        :param service_name:service_name: name of service for which to retrieve the description.
    ...
    def set_update_interval(self, service_name, interval):
        """
        Updates the api service dictionary with the timedelta specifying the length of time between api updates.
        ...
        if service_name in self._api_services:
            ...
            raise ValueError(f"{service_name} not found in api features.")
    ...
    def update_default_config(self, config):
        """
        May be called by implementing classes to add to the default configuration for its own use.
    ...
    def get_point_name_defs_file(self):
        ...
        :return: file path of a csv containing a mapping of Service_Point_Name to an optional Standard_Point_Name.
         May also optionally provide Service_Units (a Pint-parsable unit name for a point, provided by the service)
            to Standard_Units (units specified for the Standard_Point_Name by the CF standards).
         Should return None if the concrete agent does not require point name mapping and/or unit conversion.
        """

    def parse_point_name_mapping(self):
        """
        Parses point name mapping, which should contain a mapping of service points to standard points,
        ...
        try:
            ...
            _log.warning(f"Error loading mapping file ({e})")
            return None
        if mapping_file:
            try:
                ...
                for map_item in config_dict:
                    ...
                    if service_point_name:
                        standard_point_name = map_item.get("Standard_Point_Name", service_point_name)
                ...
                _log.error(f"Error parsing standard point name mapping: {error}")
                raise ValueError(f"Error parsing point name mapping from file {error}")
            finally:
                mapping_file.close()
        return point_name_mapping

    def _configure(self, _, __, contents):
        """
        ...
        self._api_calls_limit = config.get("api_calls_limit", self._api_calls_limit)
        ...
        try:
            ...
            _log.error(f"Failed to load weather agent settings with error: {e}")
            self.vip.health.set_status(STATUS_BAD, f"Configuration of weather agent failed with error: {e}")
        else:
            ...
            try:
                self._cache = WeatherCache(self._database_file,
                                           ...
                                           max_size_gb=self._max_size_gb)
                self.vip.health.set_status(STATUS_GOOD, "Configuration of weather agent successful")
            except sqlite3.OperationalError as error:
                _log.error(f"Error initializing cache: {error}")
                self.vip.health.set_status(STATUS_BAD, "Cache failed to start during configuration")

            if self.do_polling:
                if self.poll_greenlet:
                    self.poll_greenlet.kill()
                self.poll_greenlet = self.core.periodic(self.poll_interval, self.poll_for_locations)
    ...
    def validate_poll_config(self):
        ...
        if self.poll_locations:
            if not self.poll_interval:
                err_msg = "poll_interval is mandatory configuration when poll_locations are specified"
    ...
    def api_calls_available(self, num_calls=1):
        """
        Pass through to cache to check if there are api calls available to remote API
    ...
    def add_api_call(self):
        """
        Add API call timestamp entry to cache -  this method is used by concrete implementations
         for managing API call tracking for features not included in the base weather agent
        :return:
        """
    ...
    def get_current_weather(self, locations):
        ...
        Will provide cached data for efficiency if available.
        :param locations: List of location dictionary objects.
        :return: list of dictionaries containing weather data for each location.
                  result dictionary would contain all location details passed as input.
                  Weather data results  will be returned in the key 'weather_results'.
                  In case of errors, error message will be in the key 'weather_error'.

                 For example:
                 Input: [{"zipcode":"99353"}, {"zipcode":"invalid zipcode"}, {"zipcode":"99354"}]
                 Output:
        ...
        for location in locations:
            record_dict = self.validate_location_dict(SERVICE_CURRENT_WEATHER, location)
            if record_dict:
            ...
            if not record_dict.get(WEATHER_RESULTS):
                ...
                if self.api_calls_available():
                    ...
                    record_dict[WEATHER_ERROR] = "No calls currently available for the configured API key"
            result.append(record_dict)
        return result

    def get_cached_current_data(self, location):
        """
        Retrieves current weather data stored in cache if it exists
            and is current (the timestamp is within the update interval).
        :param location: location to retrieve current stored data for.
        :return: current weather data dictionary
        """
        result = location.copy()
        try:
            observation_time, data = self._cache.get_current_data(SERVICE_CURRENT_WEATHER, jsonapi.dumps(location))
            if observation_time and data:
                interval = self._api_services[SERVICE_CURRENT_WEATHER]["update_interval"]
                ...
                if current_time < next_update_at:
                    result["observation_time"] = format_timestamp(observation_time)
                    result[WEATHER_RESULTS] = jsonapi.loads(data)
        except Exception as error:
            bad_cache_message = "Weather agent failed to read from cache"
            self.vip.health.set_status(STATUS_BAD, bad_cache_message)
            status = Status.from_json(self.vip.health.get_status_json())
            self.vip.health.send_alert(CACHE_READ_ERROR, status)
            _log.error("{}. Exception:{}".format(bad_cache_message, error))
    ...
    def get_current_weather_remote(self, location):
        ...
        result = location.copy()
        try:
            observation_time, data = self.query_current_weather(location)
            observation_time, old_tz = process_timestamp(observation_time)
            if self.point_name_mapping:
                data = self.apply_mapping(data)
            if observation_time is not None:
                storage_record = [jsonapi.dumps(location), observation_time, jsonapi.dumps(data)]
                try:
                    self.store_weather_records(SERVICE_CURRENT_WEATHER, storage_record)
                except Exception:
                    bad_cache_message = "Weather agent failed to write to cache"
                    result[WEATHER_WARN] = [bad_cache_message]
                result["observation_time"] = format_timestamp(observation_time)
                result[WEATHER_RESULTS] = data
            else:
                result[WEATHER_ERROR] = "Weather api did not return any records"
        except Exception as error:
            _log.error(f"Exception getting current weather from remote {error}")
    ...
    def get_forecast_by_service(self, locations, service, service_length,
                                quantity):
        """
        RPC method returning hourly forecast weather data for each location provided.
         Will provide cached data for efficiency if available.
        ...
        :param quantity: number of time series data points of data to include with each location's records
        :param service:
        :return: list of dictionaries containing weather data for each location.
                 result dictionary would contain all location details passed as input in addition to results.
                  Weather data results  will be returned in the key  'weather_results'.
                 value of 'weather_results' will be in the format
                 [
                    [<forecast time>, <dictionary of data returned for that forecast time>],
                    [<forecast time>, <dictionary of data returned for that forecast time>],
                    ...
                ]
                 If the weather api did not return the requested number of records,
                  in addition to 'weather_results' there will also be a 'weather_warn' key.
                 In case of errors, error message will be in the key 'weather_error'.
        ...
        result = []
        for location in locations:
            record_dict = self.validate_location_dict(service, location)
            if record_dict:
                result.append(record_dict)
                continue

            # check if we have enough recent data in cache
            record_dict = self.get_cached_forecast_by_service(location, quantity, request_time, service, service_length)
            cache_warning = record_dict.get(WEATHER_WARN)
            # if cache didn't work out query remote api
            if not record_dict.get(WEATHER_RESULTS):
                if self.api_calls_available():
                    _log.debug("forecast weather from api")
                    record_dict = self.get_remote_forecast(service, location, quantity, request_time)
                    if cache_warning:
                        warnings = record_dict.get(WEATHER_WARN, [])
                        warnings.extend(cache_warning)
                        record_dict[WEATHER_WARN] = warnings
    ...
    def get_hourly_forecast(self, locations, hours=24):
        """
        RPC method for retrieving a time series of forecast data by hour.
         The amount and form of data returned is dependent upon the remote API.
        :param locations: List of location dictionaries used to build the request to the remote API
        :param hours: Optional parameter for specifying the amount of records user would like returned
        :return: list of dictionaries containing weather data for each location.
                 result dictionary would contain all location details passed as input in addition to results.
                  Weather data results  will be returned in the key  'weather_results'.
                   value of 'weather_results' will be in the format
        ...
        """
        return self.get_forecast_by_service(locations, SERVICE_HOURLY_FORECAST, 'hour', hours)

    def get_cached_forecast_by_service(self, location, quantity, request_time, service, service_length):
        """
        Retrieves forecast weather data stored in cache if it exists and is current
         (the generation timestamp is within the update interval)
        ...
        """
        record_dict = location.copy()
        interval = self._api_services[service]["update_interval"]
        # format [(generation_time, forecast_time, points), ...]
        try:
            most_recent_for_location = self._cache.get_forecast_data(service, service_length, jsonapi.dumps(location),
            ...
            if most_recent_for_location:
                ...
                if request_time < next_update_at and len(most_recent_for_location) >= quantity:
                    ...
                    while i < quantity:
                        ...
                        entry = [format_timestamp(record[1]), jsonapi.loads(record[2])]
                        location_data.append(entry)
                        i = i + 1
                    record_dict["generation_time"] = format_timestamp(generation_time)
                    record_dict[WEATHER_RESULTS] = location_data
        except Exception as error:
            bad_read_message = "Weather agent failed to read from cache"
            self.vip.health.set_status(STATUS_BAD, bad_read_message)
            status = Status.from_json(self.vip.health.get_status_json())
            self.vip.health.send_alert(CACHE_READ_ERROR, status)
            _log.error(f"{bad_read_message}. Exception:{error}")
    ...
    def query_forecast_service(self, service, location, quantity, forecast_start):
        """
        Queries a remote api service. Available services are determined per weather agent implementation
        ...
        :return: A list of time series forecast records to be processed and stored
        """
    ...
    def get_remote_forecast(self, service, location, quantity, request_time):
        ...
        :param service: Desired remote forecast service
                        - Available services are determined by concrete implementations of the weather agent
        :param location: location for which to retrieve forecast weather records
        :param quantity: number of time -series records to included with each location
        :param request_time: time at which the request for data was made, used for checking if the data is current.
        ...
        try:
            ...
            generation_time, response = self.query_forecast_service(service, location, quantity, forecast_start)
            if not generation_time:
                # in case api does not return details on when this
                # forecast data was generated
                generation_time = datetime.now(datetime.UTC)
            else:
                generation_time, old_tz = process_timestamp(generation_time)
            if self.point_name_mapping:
            ...
            for item in response:
                ...
                if item[0] is not None and item[1] is not None:
                    ...
                    storage_record = [jsonapi.dumps(location), generation_time, forecast_time, jsonapi.dumps(item[1])]
                    storage_records.append(storage_record)
                    if len(location_data) < quantity and forecast_time >= forecast_start:
            ...
            if location_data:
                try:
                    self.store_weather_records(service, storage_records)
                except Exception:
                    err_message = "Weather agent failed to write to cache"
                    result[WEATHER_WARN] = [err_message]
                result["generation_time"] = format_timestamp(generation_time)
                _log.debug(f"generation_time in result obj : {result['generation_time']}")
                result[WEATHER_RESULTS] = location_data
            else:
                result[WEATHER_ERROR] = "No records were returned by the weather query"

            if location_data and len(location_data) < quantity:
                warnings = result.get(WEATHER_WARN, [])
                warnings.append("Weather provider returned less than requested amount of data")
                result[WEATHER_WARN] = warnings
        except Exception as error:
            _log.error(f"Exception in getting remote forecast data:{type(error)}:{error}")
            result[WEATHER_ERROR] = str(error)
        return result

    def apply_mapping(self, record_dict):
        """
        Alters the weather dictionary returned by a provider to use standard point names
            specified in the agent's registry configuration file.
        (see http://cfconventions.org/Data/cf-standard-names/57/build/cf-standard-name-table.html
         for standardized weather terminology)
        ...
        for point, value in record_dict.items():
            ...
            if point in self.point_name_mapping:
                point_name = self.point_name_mapping[point]["Standard_Point_Name"]
                mapped_data[point_name] = value
                if (self.point_name_mapping[point].get("Service_Units")
                        and self.point_name_mapping[point].get("Standard_Units")):
                    mapped_data[point_name] = self.manage_unit_conversion(
                        self.point_name_mapping[point]["Service_Units"],
                        value,
                        self.point_name_mapping[point]["Standard_Units"]
                    )
            else:
    ...
    #                     record = [location, observation_time, jsonapi.loads(item[1])]
    ...
    #                     s_record = [location, observation_time, jsonapi.dumps(item[1])]
    #                     storage_records.append(s_record)
    #                     record = [location, format_timestamp(observation_time), jsonapi.dumps(item[1])]
    ...
    def poll_for_locations(self):
        """
        Called periodically by core.period to get_current_weather with the agent's polling_locations list as a parameter.
         Publishes to the corresponding entry in the agent's poll_topic_suffixes, or to /all if none are specified.
        """
    ...
    def publish_response(self, topic, publish_item):
        ...
        :param topic: topic string to send with the message bus publish
    ...
    def manage_unit_conversion(self, from_units, value, to_units):
        """
        Used to convert units from a query response to the expected standard units
    ...
    def get_cached_historical_data(self, request_name, location, date_timestamp):
        ...
        """
        return self._cache.get_historical_data(request_name, jsonapi.dumps(location), date_timestamp)

    def store_weather_records(self, service_name, records):
        """
        Generically stores weather records returned by the api to the corresponding service table in the cache database.
        :param service_name: name of the api service function, for which records will be put into the corresponding table.
...
class WeatherCache:
    ...
    def __init__(self,
        ...
        :param max_size_gb: maximum size in gigabytes of the sqlite database file,
                                useful for deployments with limited storage capacity
        :param check_same_thread: True to allow multiple threads to connect to the sqlite object, else false
    ...
    def _setup_cache(self, check_same_thread):
        ...
        _log.info(f"connected to database, sqlite version: {sqlite3.version}")
        self.create_tables()
    ...
    def create_tables(self):
        ...
        if self._calls_limit:
            try:
                ...
                if str(o).startswith("table") and str(o).endswith("already exists"):
        ...
        for service_name in self._api_services:
            ...
            try:
                ...
                if table_type == "forecast":
                    ...
                    raise ValueError(f"Invalid table type {table_type} for table {service_name}.")
                ...
                _log.error(f"Unable to create database table: {err}")
            if table_exists:
    ...
    def api_calls_available(self, num_calls=1):
        """
        :param num_calls: Number of calls requested by the agent to fulfill the user's request
        :return: True if the number of calls within the call period is less than the api calls limit, false otherwise
        """
        if num_calls < 1:
        ...
        try:
            ...
            _log.error(f"Error getting available API calls: {error}")
    ...
    def add_api_call(self):
        """
        If an API call is available given the constraints, adds an entry to the table for tracking
        ...
        try:
            ...
            cursor.executemany(insert_query, [(call,) for call in self.pending_calls])
            cursor = self._sqlite_conn.cursor()
            current_time = (get_aware_utc_now(),)
            insert_query = "INSERT INTO API_CALLS (CALL_TIME) VALUES (?);"
            ...
            _log.error(f"Error Adding api calls {type(error)}:{error}")
            return False
    ...
    def get_current_data(self, service_name, location):
        ...
        cursor = self._sqlite_conn.cursor()
        query = f"SELECT max(OBSERVATION_TIME), POINTS FROM {service_name} WHERE LOCATION = ?;"
        _log.debug(query)
    ...
    def get_forecast_data(self, service_name, service_length, location, quantity, request_time):
        """
        ...
        _log.debug(query)
        cursor.execute(query, (location, forecast_start, forecast_end, location))
    ...
    #     """
    #     Retrieves historical data over the given time period by location
    #     :param service_name: name of the api service for table lookup
    #     :param location: location to query by
    ...
    def store_weather_records(self, service_name, records):
        ...
        :return: boolean value representing whether the cache is full
        """
        cursor = self._sqlite_conn.cursor()
        ...
        cache_full = False
        if self._max_size_gb is not None and self.page_count(cursor) >= self._max_pages:
            cache_full = True
            self.manage_cache_size()
        cursor.close()
        return cache_full
